 ...
 *
 * **原寸が分かっているなら `planned` を渡す。** 計画が noop なら、寸法欄に数字が入っていても
 * 出力は元のバイト列になる（原寸を明示で打った / 拡大を頼んだ、など）。
 * `planned` は**その `srcFormat` と同じ画像の原寸**から作った物でなければならない
 * （別の画像の原寸で作った計画を渡すと、変換すべき物を素通しさせてしまう）。
 *
 * 原寸が無い / デコードできなかった（`width === 0`）ときは渡さない —— そのときは
 * 「寸法欄が空か」で見る（**甘い方へは倒さない**）。寸法欄が両方空なら計画は必ず noop なので、
 * **この落とし方は `planned` を渡した場合と同じ答えになる** ——
 * 画面はそれを使って、素通しだけは原寸の到着を待たずに出している（`renderPreview`）。
 *
